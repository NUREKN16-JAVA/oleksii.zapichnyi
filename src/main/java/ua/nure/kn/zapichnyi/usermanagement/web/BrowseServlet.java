...
public class BrowseServlet extends HttpServlet {
	...
	private void details(HttpServletRequest req, HttpServletResponse resp) throws ServletException,IOException {
        ...
        if (idStr == null || idStr.trim().length() == 0) {
            req.setAttribute("error", "User must be selected");
	...
	private void delete(HttpServletRequest req, HttpServletResponse resp)throws ServletException,IOException {
        ...
        String idStr = req.getParameter("id");
        if (idStr == null || idStr.trim().length() == 0) {
            req.setAttribute("error", "User for deleting must be selected");
            req.getRequestDispatcher("/browse.jsp").forward(req, resp);
            return;
        }
        try {
	...
	String isStr= req.getParameter("id");
	if(isStr==null||isStr.trim().length()==0) {
		...
		try {
			User user = DaoFactory.getInstance().getUserDao().find(new Long(isStr));
			req.getSession().setAttribute("user", user);
		}catch (Exception e) {
